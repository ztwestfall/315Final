...
ALU_Ops decode (const ALU_Type data) {
  if (data.instr.lsli.op == ALU_LSLI_OP) {
    // Done - 315: insert code here to print lsls instruction
    if (opts.instrs) {
      cout << "lsls r" << data.instr.lsli.rd << ", r" << setbase(10) << data.instr.lsli.imm << endl;
    }
    return ALU_LSLI;
  }
    ...
    // Done - 315: insert code here to print subs instruction
    if (opts.instrs) {
...
SP_Ops decode (const SP_Type data) {
  if (data.instr.mov.op == 2) {
    ...
    if (opts.instrs) {
       ...
       if (data.instr.add.d) {
          if ((data.instr.add.rd + 8) == 13) {
             ...
             cout << "r" << setbase(10) << (data.instr.add.rd + 8) << ", r" << setbase(10) << (data.instr.add.rd + 8) << ", ";
          }
       }
       else {
          cout << "r" << data.instr.add.rd << ", r" << data.instr.add.rd << ", ";
       }
    ...
    if (opts.instrs) {
       //cout << "cmp";
       if (data.instr.cmp.d) {
          cout << "cmp r" << setbase(10) << data.instr.cmp.rd + 8 << ", r" << setbase(10) << data.instr.cmp.rm << endl;
       }
       else {
...
LD_ST_Ops decode (const LD_ST_Type data) {
  if (data.instr.class_type.opA == LD_ST_REG_OPA) {
    if (data.instr.class_type.opB == LD_ST_OPB_LDRB) {
      // Done - 315: write code to print ldrb
      if (opts.instrs) {
      ...
      // Done - 315: write code to print strb
      if (opts.instrs) {
      ...
      // Done - 315: write code to print ldr
      if (opts.instrs) {
      ...
      // Done - 315: write code to print str
      if (opts.instrs) {
...
int decode (const LDM_Type data) {
  // Done - 315: add code to print ldm
  if (opts.instrs) {
     bool mult = false;
...
int decode (const LDRL_Type data) {
  // Done - 315: add code to print ldr
  if (opts.instrs) {
